...
class OpenDWDCrawler:
    ...
    def __init__(self, engine, init_plz_matrix=False, folder='./grb_files'):
        ...
        if init_plz_matrix:
            shape_germany = shapefile.Reader(r'./plz_geo.shp')
                ...
                for j in range(cols):
                    for key, area in plz_areas.items():
                        if area.contains(Point((lon_coordinates[i][j], lat_coordinates[i][j]))):
                            ...
                            break
            np.save(r'./plz_matrix',
                    self.plz_matrix, allow_pickle=True)
        else:
            self.plz_matrix = np.load(r'./plz_matrix.npy')
    ...
    def save_dwd_in_file(self, typ='temperature', year='1995', month='01'):
        ...
        log.info(
            f'get weather for {typ} with status code {response.status_code}')
    ...
    def read_grb_file(self, hour, counter, typ):
        ...
        df['plz'] = self.plz_matrix.reshape(-1,
                                            )[self.plz_matrix.reshape(-1,) > 0]
        df['timestamp'] = hour

        log.info(
            f'read data for typ: {typ} and hour: {counter} of {size} in month {hour.month}')
        weather_data.close()
        ...
        for date in date_range:
            try:
                ...
                for parameter in self.codecs.keys():
                    ...
                    self.save_dwd_in_file(
                        year=str(date.year), month=month, typ=parameter)
                ...
                for hour in hours:
                    ...
                    for parameter in self.codecs.keys():
                        if init_df:
                            ...
                            df[parameter] = self.read_grb_file(
                                hour, counter, parameter)[parameter]
                    ...
                    del df['timestamp']
                    log.info(
                        f'build completed for {counter} in {date.month_name()} and start import to postgres')
                    df.to_sql('cosmo', con=self.engine, if_exists='append')
                    log.info(
                        'import in postgres complete --> start with next hour')
                    counter += 1                                            # counter ++
            except Exception:
...
if __name__ == '__main__':
    import sqlite3
    engine = sqlite3.connect('dwd.db')
    crawler = OpenDWDCrawler(engine)