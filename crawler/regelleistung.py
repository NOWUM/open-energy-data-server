...
def get_date_from_sql(engine, table_name, sql):
    try:
        ...
        if ('psycopg2.errors.UndefinedTable' in str(err_obj)):
            log.info(
                    f"There does not exist a table {table_name} yet."
                )
            return None
        else:
            log.error(e)
...
def get_latest_date_if_table_exists(engine, table_name):
    sql = f"SELECT date_from FROM regelleistung.{table_name} ORDER BY date_from DESC LIMIT 1"
    return get_date_from_sql(engine, table_name, sql)
    
def get_earliest_date_if_table_exists(engine, table_name):
    sql = f"SELECT date_from FROM regelleistung.{table_name} ORDER BY date_from ASC LIMIT 1"
    return get_date_from_sql(engine, table_name, sql)
...
def database_friendly(string):
    return (string.lower()
            ...
            .replace("at_settlementcapacity_price_eur_mw", "austria_settlementcapacity_price_eur_mw")
            .replace("ch_settlementcapacity_price_eur_mw", "switzerland_settlementcapacity_price_eur_mw")
            .replace("de_settlementcapacity_price_eur_mw", "germany_settlementcapacity_price_eur_mw")
            .replace("si_settlementcapacity_price_eur_mw", "slovenia_settlementcapacity_price_eur_mw")
            .replace("be_settlementcapacity_price_eur_mw", "belgium_settlementcapacity_price_eur_mw")
            .replace("dk_settlementcapacity_price_eur_mw", "denmark_settlementcapacity_price_eur_mw")
            .replace("nl_settlementcapacity_price_eur_mw", "netherlands_settlementcapacity_price_eur_mw")
            .replace("fr_settlementcapacity_price_eur_mw", "france_settlementcapacity_price_eur_mw")
        )
...
def get_df_for_date(url, date_to_get):
    ...
    cols_to_adapt = ['total_min_capacity_price_eur_mw', 
    ...
    cols_to_drop = ['total_min_energy_price_eur_mwh', 
...
def write_concat_table(engine, table_name, new_data):
    with engine.begin() as conn:
        ...
        print(new_data['date_from'])
        complete_data = pd.concat([prev, new_data])
        complete_data.to_sql(table_name, conn, if_exists="replace", index=False)

def write_past_entries(engine, table_name, url, earliest_date, max_days_past=DEFAULT_MAX_DAYS_PAST):
    ...
    if past_days > 1:
        log.info(f"Finished writing {table_name} to Database with earliest date {earliest_date}")
    else:
        log.info(f"No past data was written for {table_name}")


def create_table_and_write_past_data(engine, url, table_name, max_days_past=DEFAULT_MAX_DAYS_PAST):
...
def add_additional_past_entries(engine, table_name, url, max_days_past=DEFAULT_MAX_DAYS_PAST):
...
def write_new_data_from_latest_date_to_today(engine, url, table_name, latest_data_date):
    ...
    if latest_data_date == (today_date - timedelta(days=1)):
        ...
        log.info(f"Finished writing new data to {table_name} with newest date being yesterday {(latest_data_date - timedelta(days=1))}")


def write_data_in_table(engine, table_name, url, max_days_past=DEFAULT_MAX_DAYS_PAST, write_additional_past_entries_if_any=True):
