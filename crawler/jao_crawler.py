...
class JaoClientWrapper:
    ...
    def get_auctions(self, corridor: str, from_date: datetime, to_date: datetime, horizon="Monthly") -> pd.DataFrame:
        from_date, to_date = string_to_timestamp(from_date, to_date)
        try:
            return self.client.query_auction_stats_months(from_date, to_date, corridor, horizon)
        except requests.exceptions.HTTPError as e:
            if e.response.status_code != 400:
                log.error(f"Error fetching auctions for corridor {corridor} from {from_date} to {to_date}: {e}")
            return pd.DataFrame()
...
def run_data_crawling(jao_client: JaoClientWrapper, from_date: datetime, to_date: datetime,
    ...
    for corridor in jao_client.get_corridors():
        for horizon in jao_client.get_horizons():
            ...
            for auction_id, auction_month in auctions_data.loc[:, ['id', 'month']].values:
                ...
                if not bids_data.empty:
                    ...
                    with db_manager.engine.begin() as connection:
                        bids_data.to_sql(table_name, connection, if_exists='append', index=False)
